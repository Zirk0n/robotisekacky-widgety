  ...
  function init(root) {
    ...
    window.fetch('/hodnoceni-obchodu/', { credentials: 'same-origin', cache: 'force-cache' })
      ...
      .then(function (html) {
        ...
        var ratingLabel = page.querySelector('.stars-label');
        var match = ratingLabel && ratingLabel.textContent.match(/(\d{2,4})\s+hodnocen/i);
        if (match) setReviewCount(parseInt(match[1], 10));
      })
      .catch(function () {});
